/* =====================================================================
   ...
   Nine modules, no shared globals beyond one namespace:
     ...
     9. highlighter  reader text highlights, notes, persisted per page
   ...
   Public surface: window.UI
     ...
     UI.highlights.list()
     UI.highlights.clear()
     UI.highlights.exportJSON()
     UI.highlights.importJSON(text)
...
(function () {
  ...
  function initJumpFlash() {
    function flash() {
      var hash = location.hash;
      if (!hash) return;
      var isFootnote = hash.indexOf('#fn') === 0;
      var isCitation = hash.indexOf('#src-') === 0;
      if (!isFootnote && !isCitation) return;
  ...
  var HL_NO_HIGHLIGHT = 'pre, code, figure, .chart, .mermaid, [data-no-highlight]';
  var HL_BLOCK_SELECTOR = '.doc p, .doc li, .doc td, .doc th, .doc h2, .doc h3, .doc blockquote';
  var HL_BLOCK_EXCLUDE = 'pre, code, figure';

  function hlStorageKey() {
    return 'ui-highlights:' + location.pathname;
  }

  function hlLoad() {
    try {
      var raw = localStorage.getItem(hlStorageKey());
  ...
  function hlSave(records) {
    try {
      localStorage.setItem(hlStorageKey(), JSON.stringify(records));
    } catch (e) {
      /* private mode or a blocked-storage browser */
    }
  }
  ...
  function hlBlocks() {
    return $$(HL_BLOCK_SELECTOR).filter(function (b) {
      return !b.closest(HL_BLOCK_EXCLUDE);
    });
  }
  ...
  function hlTextNodes(root) {
    var walker = document.createTreeWalker(root, NodeFilter.SHOW_TEXT, {
      acceptNode: function (node) {
        ...
        if (p && p.closest(HL_NO_HIGHLIGHT)) return NodeFilter.FILTER_REJECT;
  ...
  function hlBlockInfo(block) {
    var nodes = hlTextNodes(block);
    var text = '';
    var positions = [];
    nodes.forEach(function (node) {
      var start = text.length;
      text += node.nodeValue;
      positions.push({ node: node, start: start, end: text.length });
    });
    return { text: text, positions: positions };
  }

  function hlWrapSegment(node, localStart, localEnd, color) {
    var target = node;
    if (localEnd < target.length) target.splitText(localEnd);
    if (localStart > 0) target = target.splitText(localStart);
    var mark = el('mark', { class: 'hl hl-' + color });
    target.parentNode.insertBefore(mark, target);
    mark.appendChild(target);
    return mark;
  }

  function hlUnwrap(mark) {
  ...
  function hlClosestMark(node) {
    var elNode = node.nodeType === 1 ? node : node.parentElement;
    return elNode ? elNode.closest('mark.hl') : null;
  }

  function hlValidRange(range) {
    ...
    if (!startEl.closest('.doc') || !endEl.closest('.doc')) return false;
    if (startEl.closest(HL_NO_HIGHLIGHT) || endEl.closest(HL_NO_HIGHLIGHT)) return false;
    return true;
  }

  function hlMarksInRange(range) {
    var root = hlDocRoot();
    if (!root) return [];
    return $$('mark.hl', root).filter(function (m) {
      return range.intersectsNode(m);
    });
  }

  function hlSyncFromDOM() {
    var records = [];
    hlBlocks().forEach(function (block, bi) {
      var info = hlBlockInfo(block);
      $$('mark.hl', block).forEach(function (mark) {
        var textNode = mark.firstChild;
        if (!textNode || textNode.nodeType !== 3) return;
        var pos = null;
        for (var i = 0; i < info.positions.length; i++) {
          if (info.positions[i].node === textNode) {
            pos = info.positions[i];
            break;
          }
        }
        if (!pos) return;
        var colorMatch = mark.className.match(/hl-(yellow|green|blue|pink)/);
        records.push({
          b: bi,
          s: pos.start,
          e: pos.end,
          c: colorMatch ? colorMatch[1] : 'yellow',
          n: mark.dataset.note || ''
        });
      });
    });
  ...
  function hlRestoreAll() {
    ...
    records.forEach(function (rec) {
      var block = blocks[rec.b];
      if (!block) return;
      var info = hlBlockInfo(block);
      if (rec.s < 0 || rec.e > info.text.length || rec.e <= rec.s) return;
      info.positions.forEach(function (pos) {
        var s = Math.max(pos.start, rec.s);
        var e = Math.min(pos.end, rec.e);
        if (s >= e) return;
        var mark = hlWrapSegment(pos.node, s - pos.start, e - pos.start, rec.c);
        if (rec.n) {
          mark.classList.add('has-note');
          mark.dataset.note = rec.n;
          bindTip(mark, rec.n);
        }
      });
    });
  ...
  var hlToolbar, hlDefaultRow, hlNoteRow, hlNoteInput;
  var hlPendingMarks = [];

  function hlMarksForCurrentSelection() {
    var sel = window.getSelection();
    var range = sel && sel.rangeCount ? sel.getRangeAt(0) : null;
    if (range && !range.collapsed && hlValidRange(range)) {
      return hlMarksInRange(range);
    }
    var m = range ? hlClosestMark(range.startContainer) : null;
    return m ? [m] : [];
  }

  function hlBuildToolbar() {
    var toolbar = el('div', { class: 'hl-toolbar', role: 'toolbar', 'aria-label': 'Highlight' });

    var defaultRow = el('div', { class: 'hl-toolbar__row' });
    HL_COLORS.forEach(function (color) {
      var swatch = el('button', {
        class: 'hl-toolbar__swatch hl-toolbar__swatch--' + color,
        type: 'button',
      ...
      defaultRow.appendChild(swatch);
    });
    var noteBtn = el('button', { class: 'hl-toolbar__btn', type: 'button', 'aria-label': 'Add note', text: 'Note' });
    noteBtn.addEventListener('click', hlOpenNoteRow);
    var removeBtn = el('button', { class: 'hl-toolbar__btn', type: 'button', 'aria-label': 'Remove highlight', text: 'Remove' });
    removeBtn.addEventListener('click', hlRemove);
    defaultRow.appendChild(noteBtn);
    defaultRow.appendChild(removeBtn);

    var noteRow = el('div', { class: 'hl-toolbar__row', hidden: '' });
    var input = el('input', { type: 'text', class: 'hl-toolbar__input', 'aria-label': 'Highlight note', placeholder: 'Add a note' });
    input.addEventListener('keydown', function (e) {
      if (e.key === 'Enter') hlSaveNote(input.value.trim());
      if (e.key === 'Escape') hlHideToolbar();
    });
    noteRow.appendChild(input);

    toolbar.appendChild(defaultRow);
    ...
    hlToolbar = toolbar;
    hlDefaultRow = defaultRow;
    hlNoteRow = noteRow;
    hlNoteInput = input;
    document.body.appendChild(toolbar);
  }

  function hlOpenNoteRow() {
    // Capture the target marks before focusing the input, because focus
    // collapses the document selection this lookup depends on.
    hlPendingMarks = hlMarksForCurrentSelection();
    var existing = hlPendingMarks.length === 1 ? hlPendingMarks[0].dataset.note || '' : '';
    hlDefaultRow.hidden = true;
    hlNoteRow.hidden = false;
    hlNoteInput.value = existing;
    hlNoteInput.focus();
  }

  function hlHideToolbar() {
    if (!hlToolbar) return;
    hlToolbar.classList.remove('is-visible');
    hlDefaultRow.hidden = false;
    hlNoteRow.hidden = true;
  }

  function hlShowToolbarAt(range) {
    ...
    hlToolbar.style.top = rect.top + 'px';
    hlToolbar.classList.add('is-visible');
  }

  function hlHandleSelectionChange() {
    // Focusing the note input collapses the document selection, which fires
    // this same event. Ignore it while the note row is open, or the toolbar
    // hides itself the moment the reader starts typing.
    if (hlNoteRow && !hlNoteRow.hidden) return;
    var sel = window.getSelection();
    ...
    if (!hlValidRange(range)) {
      hlHideToolbar();
      return;
    }
    hlShowToolbarAt(range);
  }

  function hlApplyColor(color) {
    var sel = window.getSelection();
    ...
    var startMark = hlClosestMark(range.startContainer);
    var endMark = hlClosestMark(range.endContainer);
    if (startMark && startMark === endMark) {
      var hasNote = startMark.classList.contains('has-note');
      startMark.className = 'hl hl-' + color + (hasNote ? ' has-note' : '');
      sel.removeAllRanges();
      hlHideToolbar();
      hlSyncFromDOM();
      return;
    ...
    var root = hlDocRoot();
    var nodes = hlTextNodes(root).filter(function (n) {
      return range.intersectsNode(n);
    });
    nodes.forEach(function (n) {
      var localStart = n === range.startContainer ? range.startOffset : 0;
      var localEnd = n === range.endContainer ? range.endOffset : n.length;
      if (localStart >= localEnd) return;
      hlWrapSegment(n, localStart, localEnd, color);
    });
    sel.removeAllRanges();
    hlHideToolbar();
    hlSyncFromDOM();
  ...
  function hlRemove() {
    var marks = hlMarksForCurrentSelection();
    marks.forEach(hlUnwrap);
    var sel = window.getSelection();
  ...
  function hlSaveNote(text) {
    var marks = hlPendingMarks;
    marks.forEach(function (m) {
      if (text) {
        ...
        delete m.dataset.note;
        m.classList.remove('has-note');
      }
    });
    hlSyncFromDOM();
    var sel = window.getSelection();
  ...
  function hlClear() {
    var root = hlDocRoot();
    if (root) {
      $$('mark.hl', root).forEach(hlUnwrap);
    }
    hlSave([]);
  }

  function hlExportJSON() {
    var text = JSON.stringify(hlLoad(), null, 2);
    copyText(text).then(
      function () {
  ...
  function hlImportJSON(text) {
    ...
    var root = hlDocRoot();
    if (root) $$('mark.hl', root).forEach(hlUnwrap);
    hlSave(parsed);
    hlRestoreAll();
  }
  ...
  UI.highlights = {
    list: hlList,
    clear: hlClear,
    exportJSON: hlExportJSON,
    importJSON: hlImportJSON
  };
  ...
  function initHighlighter() {
    ...
    document.addEventListener('mouseup', function (e) {
      ...
      setTimeout(hlHandleSelectionChange, 0);
    });
    document.addEventListener('selectionchange', hlHandleSelectionChange);
  ...
  function boot() {
    ...
    initBackToTop();
    initSearch();
    // SITE DEVIATION: the reader text highlighter is off. Re-syncing this
    // file from .claude/skills/ui-design/assets/script.js re-enables it.
    // initHighlighter();
    initIcons();
